 ...
 * Identical to the one in CreateTeacherModal — the school dependency is the
 * same for student registration (backend requires schoolId when type==="student").
 */
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-[13px] text-[#666]">
            Students belong to a school. Create yours below to continue.
          </p>
        </div>
      </div>
...
export default function CreateStudentModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !studentId.trim() || !password.trim()) {
      setFormError("Name, Student Id, and Password are required.");
      return;
    }
  ...
  // Treat any school query error or missing _id as "school missing" — the
  // backend wraps a 404 inside a 500 in some paths.
